function splitKana(str) {
  // Split into kana units for the crossword.
  // Notably, treat ydon (e.g. きょ / しゅ) as a SINGLE cell by combining
  // an i-row kana (き/し/ち/に/ひ/み/り etc.) + small ゃ/ゅ/ょ.
...
// Minimal romaji -> hiragana mapping (enough for typical kana input).
const ROMAJI_MAP = {
  a: 'あ', i: 'い', u: 'う', e: 'え', o: 'お',
...
function convertRomajiIncremental(buffer) {
  ...
  while (i < s.length) {
    ...
    // Only accept letters (drop others).
    if (!/[a-z]/.test(ch)) {
      i++;
      continue;
    }

    // Special: handle "n" ambiguity.
    if (ch === 'n') {
      const next = s[i + 1];
      if (!next) {
        // trailing 'n' might be start of 'na/ni...' so keep it.
      ...
      if (isVowel(next) || next === 'y') {
        // part of a syllable like na/nya; keep processing as normal.
      } else {
    ...
    // Can't match at this position.
    // Preserve the remainder so partial sequences like "ky" or "sh" stay intact
    // until the user types enough to form a valid syllable (e.g. "kyo", "shi").
    return { kana: out, rest: s.slice(i) };
  }
